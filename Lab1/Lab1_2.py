...
if len(sys.argv) > 4:
    ...
    elif sys.argv[1] == '--files':     # если указан только files с количеством файлов равных или больше 3
        path = Path.cwd()
...
if len(sys.argv) == 1: # если ничего не указано
    path = Path.cwd()
...
elif sys.argv[1] == '--files': # если указан только files с количеством файлов меньше 3
    path = Path.cwd()
    i = 2  # аргумент, с которого начинаются названия файлов
    while len(sys.argv[i]) > 1 and i < len(sys.argv) - 1: # добавление всех файлов в список на проверку
        files_not_in_dir.append(sys.argv[i])
